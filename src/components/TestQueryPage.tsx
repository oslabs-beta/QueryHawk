    ...
    try {
      ...
      const response = await fetch('http://localhost:4002/api/query-metrics', {
        ...
        headers: {
          ...
          Authorization: `Bearer ${token}`, // Add the token to the headers gitHub OAuth. Required with auth middelware
        },
    ...
    <ThemeProvider theme={darkTheme}>
      <Box sx={{ bgcolor: 'background.default', minHeight: '100vh' }}>
        ...
        <Container maxWidth='xl' sx={{ mt: 4 }}>
          {/* If token is missing will give alert and notify user to log. Not sure to either redirect immediately or use this method. */}
          {!isAuthenticated ? (
            <Alert
              severity='warning'
              action={
